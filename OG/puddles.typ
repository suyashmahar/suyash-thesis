In this paper, we argue that current work has failed to provide a comprehensive and maintainable in-memory representation for persistent memory.
...
We propose #emph[Puddles];, a new persistent memory abstraction, to solve these problems. Puddles provide application-independent recovery after a power outage; they make recovery from a system failure a system-level property of the stored data rather than the responsibility of the programs that access it. Puddles use native pointers, so they are compatible with existing code. Finally, Puddles implement support for sharing and shipping of PM data between processes and systems without expensive serialization and deserialization.

Compared to existing systems, Puddles are at least as fast as and up to 1.34$times$ faster than PMDK while being competitive with other PM libraries across YCSB workloads. Moreover, to demonstrate Puddles' ability to relocate data, we showcase a sensor network data-aggregation workload that results in a 4.7$times$ speedup over PMDK.

#block[
\<ccs2012\> \<concept\> \<concept\_id\>10002951.10003152.10003153.10003158\</concept\_id\> \<concept\_desc\>Information systems~Storage class memory\</concept\_desc\> \<concept\_significance\>500\</concept\_significance\> \</concept\> \<concept\> \<concept\_id\>10010583.10010786.10010809\</concept\_id\> \<concept\_desc\>Hardware~Memory and dense storage\</concept\_desc\> \<concept\_significance\>500\</concept\_significance\> \</concept\> \<concept\> \<concept\_id\>10010520.10010575.10010577\</concept\_id\> \<concept\_desc\>Computer systems organization~Reliability\</concept\_desc\> \<concept\_significance\>500\</concept\_significance\> \</concept\> \<concept\> \<concept\_id\>10011007.10011006.10011072\</concept\_id\> \<concept\_desc\>Software and its engineering~Software libraries and repositories\</concept\_desc\> \<concept\_significance\>500\</concept\_significance\> \</concept\> \</ccs2012\>
...
However, existing PM programming systems are built on a patchwork of modifications to the memory-mapped file interface and thus make several compromises in how persistent data is accessed. These systems use custom pointer formats, handle logging through ad-hoc mechanisms, and implement recovery using diverse but incompatible logging and transactional semantics.
...
(a)~#emph[Application-independent crash-recovery];: PM recovery after a crash in Puddles completes before #emph[any] application accesses the data. Recovery succeeds even if the application writing data at the time of the crash is absent after restart, no longer has the write permissions, or was just one of the multiple applications updating the data at the time of the crash.

(b)~#emph[Native pointers for PM data];: Puddles use native pointers and, thus, allow code written with other PM libraries or non-PM-aware code to read and reason about it. Pointers are a fundamental and universal tool for in-memory data structure construction. Changing their implementation for PM adds runtime overhead of translation, requires specialized code to read PM data, and stymies software engineering tools (#emph[e.g.];, compilers and debuggers do not understand custom pointer formats used by PM libraries).

(c)~#emph[Relocatability];: Puddles can transparently relocate data to avoid any address conflicts and thereby enable sharing and relocation of PM data between machines.
...
& & & & & & \
& & & & & & \
PMDK@pmdk & & $bold(times.big)$& $bold(times.big)$& $bold(times.big)$& & $bold(times.big)$ \
TwizzlerOS@bittman2020twizzler & & $bold(times.big)$& $bold(times.big)$& & & $bold(times.big)$ \
Mnemosyne@mnemosyne & & & $bold(times.big)$& $bold(times.big)$& $bold(times.big)$& \
NV-Heaps@nvheaps & & $bold(times.big)$& $bold(times.big)$& $bold(times.big)$& & $bold(times.big)$ \
Corundum@corundum & & $bold(times.big)$& $bold(times.big)$& $bold(times.big)$& & $bold(times.big)$ \
Atlas@atlas & & & $bold(times.big)$& $bold(times.big)$& & $bold(times.big)$ \
Clobber-NVM@clobbernvm & & & $bold(times.big)$& $bold(times.big)$& & $bold(times.big)$ \
#strong[Puddles] & #strong[] & #strong[] & #strong[] & #strong[] & #strong[] & #strong[] \
...
To understand the limitations and fragmented feature space of PM libraries, Table~@tab:related-pm-libraries compares several PM programming libraries across multiple axes. Puddles is the only PM programming library that supports features like application-independent recovery, object relocatability (moving individual objects in a process's address space), region relocatability (moving groups of objects), and the ability to modify any global PM data in a transaction, features that users expect from a mature storage system.
...
<pm-pointers-are-restrictive-and-inflexible>
#figure([#block[
  #box(image("Figures/ptr-chasing-benchmarks.pdf", width: \columnwidth))
  ]],
  caption: [
...
<puddles-implementation>
The Puddle system consists of three major system components (Figure~@fig:puddles-arch) that work together to provide application support for mapping and managing puddles.

#figure([#block[
  #box(image("Figures/architecture.pdf", width: \columnwidth))
  ]],
  caption: [
...
#figure([#block[
  #box(image("Figures/puddles_arch_overview.pdf", width: \columnwidth))
  ]],
  caption: [
...
#figure([#block[
  #box(image("Figures/pmdk-vs-libpuddles.pdf", width: \columnwidth))
  ]],
  caption: [
...
#figure([#block[
  #box(image("Figures/logspace_and_logs.pdf", width: 3in))
  ]],
  caption: [
...
==== Example hybrid logging implementation
<example-hybrid-logging-implementation>
...
#block[
#block[
#box(image("Figures/hybrid-log-new.pdf", width: 100%))

]
]
==== Transaction commit.
<transaction-commit.>
The application (perhaps via a library like `Libtx`) commits a transaction in three stages and without communicating with puddled. Some logging schemes may not need all three stages. For instance, a purely redo-based logging scheme could skip the undo stage. This is shown in Figure~@fig:hybrid-log-desc with the `sfence` and `clwb` ordering and the sequence numbers used for delineating the stages (elaborated on later in this section). The first two stages work on the undo and redo logs, respectively, and the final stage marks the log as invalid. These three stages are:

+ #emph[Stage 1, Flush undo logged locations] (Figure~@fig:hybrid-log-desca). `Libpuddles` goes through the undo log entries and makes the corresponding locations durable on the PM.

+ #emph[Stage 2, Apply the redo log] (Figure~@fig:hybrid-log-descb). Once all the undo-logged locations are flushed to PM, `Libpuddles` starts copying new data from the redo logs. Redo logged locations were unchanged before the commit, so, `Libpuddles` copies the new data from the log entry to the corresponding memory location.

+ #emph[Stage 3, TX complete] (Figure~@fig:hybrid-log-descc). The transaction is complete, and all changes are durable. The log is marked as invalid.
...
+ #emph[Recovery from crash on or before Stage 1 (Rollback)];. First, `Puddled` applies all valid undo-log entries in reverse order.
  ...
  At "Stage 1" in Figure~@fig:hybrid-log-desc, the undo log entries are still valid, but some locations covered by the undo entries might have been modified before the crash. Thus, on recovery, `Puddled` rolls back the transaction by replaying the undo log in reverse. Which entries to apply and in what order is clear from the sequence range, sequence numbers, and recovery order fields in the log entries.
...
After a crash, the daemon compares each log entry's sequence number with the log's sequence range to identify the active stage before the crash. In the hybrid logging example, the application can assign sequence number 1 to the undo log entries and 3 to the redo log entries (Figure~@fig:hybrid-log-desc). This assignment allows the `Libpuddles` to mark stages 1, 2, and 3 by setting the log's sequence range to $(0 \, 2)$ to only replay the undo logs, $(2 \, 4)$ to only replay the redo logs, and $(4 \, 4)$ to replay neither.

In Puddles, regardless of whether an entry is an undo or redo log entry, to apply an active log entry, the daemon needs to only copy the entry's content to the corresponding memory location. For example, in undo logging, the entry contains the old data, copying its contents would "undo" the memory location. Similarily, copying contents of a redo log entry would apply the entry, resulting in a "redo" operation.
...
#figure(image("Figures/consistency-api.pdf", width: \linewidth),
  caption: [
...
<referential-integrity>
While the referential integrity of exported data is a concern, applications are expected to only export self-contained pools. In Puddles, this can be accomplished by limiting inter-pool links or using programming language support to prevent inter-pool pointers.
...
#figure(
  align(center)[#table(
    ...
    [1-2 #strong[CPU & HW Thr.];], [Intel Xeon 6230 & 20], [], [#strong[Linux Kernel];], [v5.4.0-89],
    [#strong[DRAM / PM];], [93~GiB / 6$times$128~GiB], [], [#strong[Build system];], [gcc 10.3.0],
    [1-2], [], [], [], [],
  )]
...
Table~@tab:sysconfig lists the system configuration. For all experiments, we use Optane DC-PMM in App Direct Mode. All workloads use undo-logging for both the application and allocator data logging except for the linked list workload.
...
#strong[Relocatability primitives.] On a request to export a pool, `Puddled` creates copies of the puddles and the associated metadata (e.g., pointer maps). Data export cost, therefore, scales linearly with the size of the PM data and includes a constant overhead per puddle. Exporting a pool takes 0.3~s for 16~B and 0.5~s for 16~MiB of PM data in our implementation. Importing data, on the other hand, is nearly free, as it only includes registering the imported puddles with the daemon (1.5~ms for both 16~B and 16~MiB). After import, if the imported data conflicts with an existing range, the puddle system automatically rewrites all the pointers in the mapped puddle. During pointer rewrite, every pointer in the pool must be visited, so runtime scales linearly with the number of pointers in the pool. Rewriting pointers takes 0.2~ms for 20 pointers, 1.6~ms for 2000 pointers, and 0.5s for 2~million pointers.

#strong[Correctness Check.] To ensure the correctness of Puddles' logging implementation, we inject crashes into Puddles' runtime and run system-supported recovery. We do this for undo and redo logging and find that Puddles recover application data to a consistent and correct state every time.

== Workload evaluation
<subsec:workload-evaluation>
#figure([#block[
  #box(image("Figures/linkedlist.pdf", width: \columnwidth))
  ]],
  caption: [
...
#strong[B-Tree] Figure~@fig:btree-perf shows the performance of an identical order 8 B-Tree implementation in PMDK, Puddles, and Romulus. Both the keys and the values are 8 bytes. Similar to the Linked List benchmark, Puddles perform as fast as or better than PMDK across the three operations while being competitive with Romulus. In summary, Puddles' native-pointer results in a much faster (3.1$times$) performance over PMDK for search operations.

#figure([#block[
  #box(image("Figures/simplekv.pdf", width: \columnwidth))
  ]],
  caption: [
...
#figure(image("Figures/data-agg-breakdown.pdf", width: \linewidth),
  caption: [
...
<acknowledgement>
We would like to thank Terence Kelly and the anonymous reviewers for their helpful comments. This work was supported in part by NSF award 2011212 and the ACE Center for Evolvable Computing, one of the seven centers in JUMP 2.0, a Semiconductor Research Corporation (SRC) program sponsored by DARPA.
...
The provided artifact aims to verify the performance claims in the paper's Figures 9, 10, and 11.

== Description & Requirements
<description-requirements>
=== How to access
<how-to-access>
The artifact is available on Zenodo: #link("https://doi.org/10.5281/zenodo.8400339")

=== Hardware dependencies
<hardware-dependencies>
The artifact requires an x86-64 machine and at least one Intel Optane DC-PMM mounted at `/mnt/pmem0/`.
...
Ubuntu 20.04#footnote[with no existing PMDK installation]
...
=== Benchmarks
<benchmarks>
The artifact uses the following benchmarks: (a) YCSB, (b) Examples from PMDK.

== Setup
<setup>
+ The artifact requires enabling userfaultfd: \
  `echo 1 | sudo tee /proc/sys/vm/unprivileged_userfaultfd`

+ Optane DC-PMM should be mounted at `/mnt/pmem0` with `dax` support. To check, run: \
  `mount | grep /mnt/pmem0`

== Evaluation Workflow
<evaluation-workflow>
=== Using Docker Containers
<using-docker-containers>
The artifact can be run inside a docker container but needs access to the host's `/mnt/pmem0` and `/dev/shm`.
...
#block[
#strong[NOTE];: To skip dependency installation, the \
`setup-and-run.sh` script can be passed an optional `–skip-deps` argument: \
`./setup-and-run.sh –skip-deps # Run evaluation only`

]
Although libpuddles is automatically compiled by this script, to manually compile it, checkout \
`artifact-root/libpuddles/README.md`.

=== Major Claims
<major-claims>
The artifact provided reproduces the following claims from the paper:

+ #strong[Figure 9];. Puddles' performance comparison against PMDK and Romulus for singly linked list.

+ #strong[Figure 10];. Performance comparison of Puddles, PMDK, and Romulus's implementation of an order 8 Btree.

+ #strong[Figure 11];. KV Store implementation and performance comparison using different PM programming libraries, evaluated using YCSB workloads.

=== Experiments
<experiments>
The script `./setup-and-run.sh` runs three experiments to reproduce the three major claims. To manually run them, run the following commands:
...
<notes-on-libpuddles-scriptsrun.sh-scripts>
...
+ `LIBPUDDLES_PUDDLED_PORT`: Port to use for the puddled daemon

+ `LIBPUDDLES_DIR`: Directory with libpuddles' source (`artifact-root/libpuddles` for this artifact)

+ `ROMULUS_DIR`: Directory with Romulus's source \
  (`artifact-root/OneFile-romulus` for this artifact)
...
+ #strong[Run experiments]

+ Run `libpuddles-scripts/run/linkedlist.sh` and write results to `libpuddles-scripts/data/linkedlist.csv`.

+ #strong[Plot Figure 9] using the linkedlist data and write to `plots/ae/fig9.pdf` and `plots/ae/fig9.pdf`

+ Run `libpuddles-scripts/run/btree.sh` and write results to `libpuddles-scripts/data/btree.csv`.

+ #strong[Plot Figure 10] using the btree data and write to `plots/ae/fig10.pdf` and `plots/ae/fig10.pdf`

+ Run `libpuddles-scripts/run/simplekv.sh` and write results to `libpuddles-scripts/data/simplekv.csv`.

+ #strong[Plot Figure 11] using the simplekv data and write to `plots/ae/fig10.pdf` and `plots/ae/fig11.pdf`
